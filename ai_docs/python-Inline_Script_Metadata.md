...
[inline-script-metadata.rst](https://github.com/pypa/packaging.python.org/blob/main/source/specifications/inline-script-metadata.rst)
...
This specification defines a metadata comment block format (loosely inspired by [reStructuredText Directives](https://docutils.sourceforge.io/docs/ref/rst/directives.html)).
...
The `[tool]` table MAY be used by any tool, script runner or otherwise, to configure behavior. It has the same semantics as the [:ref:\`\[tool\] table in pyproject.toml <pyproject-tool-table>\`](https://github.com/pypa/packaging.python.org/blob/main/source/specifications/inline-script-metadata.rst#id3).

The top-level fields are:

-   `dependencies`: A list of strings that specifies the runtime dependencies of the script. Each entry MUST be a valid [:ref:\`dependency specifier <dependency-specifiers>\`](https://github.com/pypa/packaging.python.org/blob/main/source/specifications/inline-script-metadata.rst#id5).
-   `requires-python`: A string that specifies the Python version(s) with which the script is compatible. The value of this field MUST be a valid [:ref:\`version specifier <version-specifiers>\`](https://github.com/pypa/packaging.python.org/blob/main/source/specifications/inline-script-metadata.rst#id7).
...
def add(script: str, dependency: str) \-> str:
    ...
    return script\[:start\] + new\_content + script\[end:\]
```
